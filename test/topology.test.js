...
describe('对着真实档案', () => {
  ...
  test('**分叉的目录照样解析，而且结果恰好是并集**', (t) => {
    ...
    const merged = parse(all);
    const chain = parse(openAll(DL).filter((s) => s.bundleId !== topo.roots[topo.roots.length - 1]));
    const lone = parse(openAll(DL).filter((s) => s.bundleId === topo.roots[topo.roots.length - 1]));
